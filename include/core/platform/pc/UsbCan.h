...
#ifndef SGLOGGER_USBCAN_H
#define SGLOGGER_USBCAN_H
...
namespace core { namespace can {
        ...
        public:
            ...
            virtual void setFilter(canid_t filter) {
                _filter = filter;
                init();
            }

            virtual canid_t getFilterMask() {
                return _mask;
            }

            virtual void setFilterMask(canid_t mask) {
                _mask = mask;
                init();
            }

            virtual void send(CanFrame& canFrame) {
...
#define MAX_FRAME_SIZE 13
            ...
            virtual void send(shared_ptr<CanFrame> canFrame) {
                send(*canFrame);
            }
            ...
            void receive() {
                ...
                int32_t readResult = _serialPort.read(&byte, 1);
